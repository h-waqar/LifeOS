...
if (typeof window !== "undefined" && !process.env.VITEST) {
  throw new Error(
    "Security violation: Notes [id] API route cannot be executed in the browser."
  );
}
...
const MAX_NOTES_BODY_SIZE = 128 * 1024;
...
/**
 * GET /api/notes/[id]
 * Retrieves a single note with outgoing links and backlinks.
 */
...
export async function PUT(
  ...
  try {
    ...
    console.error("PUT /api/notes/[id] unexpected error:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500, headers: SECURITY_CACHE_HEADERS }
    );
...
/**
 * DELETE /api/notes/[id]
 * Archives or permanently deletes a note.
 */
export async function DELETE(
  ...
  try {
    ...
    await deleteNote(user.id, id, hardDelete);

    return NextResponse.json(
      { success: true, message: hardDelete ? "Note permanently deleted" : "Note archived" },
      { status: 200, headers: SECURITY_CACHE_HEADERS }
    );
  } catch (error) {
    if (error instanceof AuthenticationError) {
    ...
    console.error("DELETE /api/notes/[id] unexpected error:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500, headers: SECURITY_CACHE_HEADERS }
    );
